...
export const Home = () => {
  ...
  const [vitesse, setVitesse] = useState(150); // plus grand = plus lent

  useEffect(() => {
    const currentPhrase = phrases[indexPhrase];

    if (!estEnTrainDeSupprimer && indexLettre === currentPhrase.length) {
      setTimeout(() => setEstEnTrainDeSupprimer(true), 1500); // pause avant effacement
      return;
    }
  ...
  <span className="border-r-2 border-violet-400 animate-pulse ml-1"></span>
          ...
          <div className="flex justify-center items-center space-x-6">
            ...
            <a
              href="#contact"
              className="border border-violet-500/50 text-violet-500 py-3 px-6 rounded font-medium transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_0_15px_rgba(59, 130, 246, 0.2)] hover:bg-blue-500/10"
            >
              Me contacter
            </a>
            ...
            <div className="flex items-center space-x-4 ml-4">
              <a
                href="https://www.linkedin.com/in/ayoub-dahbi-52b3411b1"
                target="_blank"
